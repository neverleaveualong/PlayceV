...
//     const businessNumber = `${String(currentPrefix).padStart(3, '0')}-${String(currentMiddle).padStart(2, '0')}-${String(currentSuffix).padStart(5, '0')}`;
//     numbers.push({ businessNumber: businessNumber, isValid: true });

//     // 다음 번호를 위해 증가
//     currentSuffix++;
//     if (currentSuffix > 99999) { // suffix가 최대치를 넘으면 middle 증가
//       currentSuffix = 10000;
//       currentMiddle++;
//       if (currentMiddle > 99) { // middle이 최대치를 넘으면 prefix 증가
//         currentMiddle = 10;
//         currentPrefix++;
//         if (currentPrefix > 999) { // prefix가 최대치를 넘으면 (매우 드물지만)
//           // 더 이상 유효한 범위를 찾을 수 없음을 알립니다.
//           console.warn('⚠️ 경고: 사업자등록번호 생성 범위가 부족하여 더 이상 고유한 번호를 생성할 수 없습니다.');
//           break; // 루프를 중단합니다.
//         }
//       }
//     }
...
// const businessNumbers = [
//   ...generateBusinessNumbers(500) // 추가로 500개의 유효한 사업자등록번호 생성
// ];
...
export const seedBusinessNumbers = async () => {
  ...
  for (const item of businessNumbers) {
    const existing = await businessNumberRepo.findOneBy({ businessNumber: item.businessNumber });
    if (!existing) {
