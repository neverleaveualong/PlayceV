...
export const authenticate = async (
  ...
  log("[Auth] Authorization header:", authHeader);
  ...
  log("[Auth] Extracted token:", token);
  ...
  try {
    ...
    log("[Auth] 인증 성공, userId:", decoded.userId);
    next();
  } catch (err) {
...
export const optionalAuthenticate = async (
  ...
  const authHeader = req.headers.authorization;
  log("[OptionalAuth] Authorization header:", authHeader);

  if (!authHeader) {
    log("[OptionalAuth] 토큰 없음, 다음 미들웨어로 넘어감");
    return next(); // 토큰이 없으면 그냥 통과
  }

  const token = authHeader.split(" ")[1];
  log("[OptionalAuth] Extracted token:", token);
  ...
  try {
    ...
    log("[OptionalAuth] JWT 디코딩 결과:", decoded);

    // Redis에 토큰 존재 여부 확인
    const redisKey = `login:token:${token}`;
    const storedUserId = await getCache(redisKey);
    log(`[OptionalAuth] Redis에서 토큰 조회: ${storedUserId}`);

    // 수정: storedUserId 문자열 변환 비교
    if (!storedUserId || storedUserId.toString() !== decoded.userId.toString()) {
      log("[OptionalAuth] Redis 토큰 유효하지 않음, 인증정보 없이 통과");
      return next();
    }
    ...
    if (user) {
      req.user = { userId: decoded.userId };
      log("[OptionalAuth] 사용자 정보 조회 성공, req.user 설정");
    }
