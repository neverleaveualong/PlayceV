...
console.log(`환경 변수 : .env${process.env.NODE_ENV ? `.${process.env.NODE_ENV} 파일` : " 파일"}, log(${process.env.LOG_ENABLED !== 'false' ? '활성화' : '비활성화'}), 식당 관련 지역 데이터(${process.env.MOCK_GEOCODING !== 'false' ? 'mock 데이터 사용' : 'kakaoAPI 사용'})`);
