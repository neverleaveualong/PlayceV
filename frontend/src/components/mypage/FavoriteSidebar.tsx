  ...
  return (
    <section className="w-full">
      <ul className="flex flex-col gap-2.5 px-4 pb-4 pt-3">
        {favorites.map((store) => {
          const broadcasts = broadcastsByStore.get(store.store_id) ?? [];

          return (
            <li
              key={store.store_id}
              className="group relative rounded-2xl bg-white shadow-[0_1px_4px_rgba(0,0,0,0.06)] hover:bg-primary4/30 cursor-pointer transition-all duration-150"
            ...
            >
              ...
              {broadcasts.length > 0 && (
                <div className="px-3 pb-3">
                  <div className="bg-primary4/20 rounded-xl px-3 py-2.5">
                    <div className="flex items-center gap-1.5 mb-2">
                      <FiTv className="text-primary5 text-[11px]" />
                      <span className="text-[11px] font-semibold text-primary5">
                    ...
                    <ul className="space-y-1.5">
                      {broadcasts.slice(0, 3).map((b) => (
                        <li
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            openDetail(b.store.store_id, "중계");
                          }}
                        >
                          <span className="text-gray-400 flex items-center gap-0.5 flex-shrink-0">
                            <FiClock className="text-[9px]" />
                            {formatDate(b.match_date)} {b.match_time}
                          </span>
                          <span className="bg-gray-200/80 text-gray-600 rounded px-1.5 py-0.5 text-[10px] font-medium flex-shrink-0">
                            {b.league}
                          </span>
                          {b.team_one && b.team_two ? (
                            ...
                            <span className="text-gray-500 truncate">{b.sport}</span>
                          )}
                        </li>
                      ))}
                      {broadcasts.length > 3 && (
                        <li className="text-[10px] text-gray-400 pl-0.5">
                          +{broadcasts.length - 3}건 더보기
                        </li>
                      )}
                    </ul>
                  </div>
